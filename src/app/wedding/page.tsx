            ...
            <Link key={c.title} href={c.href} className="group flex-1 relative border border-white/10 hover:border-accent/50 p-6 md:p-8 flex flex-col justify-end transition-all duration-[3000ms] ease-in-out bg-black/40 overflow-hidden hover:flex-[2.5]">
              ...
              <div className="relative z-10 w-full overflow-hidden flex flex-col justify-end h-full">
                <div className="mt-auto">
                  <h3 className="text-2xl md:text-3xl font-serif text-[#E8E0D0] leading-tight">{c.title}</h3>
                  <p className="text-[10px] md:text-xs uppercase tracking-[0.2em] text-accent mt-2 transition-opacity duration-[1500ms] mb-6">{c.desc}</p>
                  <span className="inline-block text-[10px] uppercase tracking-[0.3em] text-black bg-accent px-4 py-2 opacity-0 group-hover:opacity-100 transition-all duration-[1500ms] delay-200 hover:bg-white">
                    View Collection
                  </span>
                </div>
              </div>
