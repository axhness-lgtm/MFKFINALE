...
export default function FormalsBlazersPage() {
  const items = Array.from({ length: 24 }).map((_, i) => ({
    id: `21${String(i + 1).padStart(2, '0')}`,
    name: `${'Tailored Blazers'.replace(/s$/, '')} No. ${i + 1}`,
    ...
    <div className="min-h-screen pt-40 pb-24 bg-[#0a0a09] relative">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <FadeIn className="text-center space-y-6 mb-16 max-w-4xl mx-auto">
          <h1 className="text-4xl md:text-5xl font-serif font-light text-[#E8E0D0] uppercase tracking-widest">Tailored Blazers</h1>
          <p className="text-lg text-accent font-serif italic">Bridging the definitive line between smart and casual.</p>
        </FadeIn>
        ...
        <FadeIn delay={100} className="border-t border-white/10 pt-8 mb-16 text-center max-w-3xl mx-auto">
          <p className="text-sm text-white/50 leading-relaxed font-light tracking-wide" style={{ fontFamily: 'Helvetica, Arial, sans-serif' }}>
            Unstructured or lightly padded shoulders, patch pockets.
          </p>
        </FadeIn>
