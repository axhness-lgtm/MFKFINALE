        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-16 md:gap-8 lg:gap-16">
          {testimonials.map((t, idx) => (
            <FadeIn key={idx} delay={idx * 200} className="space-y-10 relative">
              <div className="absolute -top-4 -left-3 text-accent text-6xl font-normal select-none" style={{ fontFamily: '"Ballet", cursive' }}>"</div>
              <p className="text-xl font-headline font-light leading-relaxed italic text-white/80 relative z-10">
                {t.quote}
              </p>
              <div className="space-y-1 pt-6 border-t border-white/20">
                <h4 className="text-[10px] uppercase tracking-[0.3em] font-bold text-white">{t.author}</h4>
                <p className="text-[9px] uppercase tracking-[0.2em] text-white/60 font-medium">{t.role}</p>
              </div>
            </FadeIn>
          ))}
        </div>
